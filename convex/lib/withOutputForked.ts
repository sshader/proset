...
import { EmptyObject } from "convex-helpers/index";
...
>(
  ...
  function customQueryBuilder(fn: any): any {
    if ("args" in fn) {
      ...
      const convexOutputValidator = zodToConvex(fn.output ?? z.any())
      const wrappedFn = query({
        args: {
          ...convexValidator,
          ...inputArgs,
        },
      ...
      // @ts-expect-error patch on exportOutput like exportArgs
      wrappedFn.exportOutput = () => JSON.stringify(convexOutputValidator.json)
      return wrappedFn
    }
