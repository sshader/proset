...
async function analyzeModule(filePath: string): Promise<AnalyzedFunctions> {
    ...
    for (const [name, value] of Object.entries(importedModule)) {
      ...
      if (isPublic && isInternal) {
        console.debug(`Skipping function marked as both public and internal: ${name}`);
        continue;
      } else if (isPublic) {
...
export function* walkDir(
    ...
    for (const dirEntry of fs.readdirSync(dirPath, { withFileTypes: true }).sort()) {
...
function generateFile(paths: string[], filename: string, isNode: boolean) {
  ...
  for (const p of paths) {
    const safeModulePath = importPath(p)
        .replace(/\//g, "_")
        .replace(/-/g, "_");
...
async function main(convexDir: string, analyzeDir: string) {
    ...
    execSync("npx convex deploy --dry-run -y --debug-bundle-path /tmp/debug_bundle_path")
    const outputStr = fs.readFileSync("/tmp/debug_bundle_path", { encoding: "utf-8"});
    ...
    for (let i = 0; i < isolatePaths.length; i += chunkSize) {
      ...
      generateFile(chunk, `${convexDir}/${analyzeDir}/group${chunkNumber}.ts`, false)
      chunkNumber += 1
    }
    for (let i = 0; i < nodePaths.length; i += chunkSize) {
      const chunk = nodePaths.slice(i, i + chunkSize);
      generateFile(chunk, `${convexDir}/${analyzeDir}/group${chunkNumber}.ts`, true)
      chunkNumber += 1
    }
    ...
    for (let i = 0; i < chunkNumber; i += 1) {
      const result = execSync(`npx convex run ${analyzeDir}/group${i}:default`).toString()
      console.log(result)
      fullResults = {
        ...fullResults,
        ...JSON.parse(result)
